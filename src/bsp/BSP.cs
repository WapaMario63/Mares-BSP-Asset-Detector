...
namespace MareAssetDetector.Pack
{
	...
	class BSP
    {
        ...
        private Tuple<string, string, string>? ParseIO(string property)
        {
            ...
            if (io.Length != 5)
            {
                Logger.LogError($"Failed to decode IO, ignoring: {property}\n");
                return null;
            }
            ...
            if (targetInput == "AddOutput")
            {
                ...
                if (parameter.Contains(':'))
                {
                    ...
                    if (splitIo.Length < 3)
                    {
                        Logger.LogError($"Failed to decode AddOutput, format may be incorrect: {property}\n");
                        return null;
                    }
